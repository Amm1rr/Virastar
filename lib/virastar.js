...
(function (name, global, definition) {
  ...
  function fixPerfixSpacing(text) {
    ...
    return text
      .replace(
        /((\s|^)ن?می|(\s|^)ام|(\s|^)ات|(\s|^)اش|(\s|^)ای|(\s|^)اید|(\s|^)ایم|(\s|^)اند|(\s|^)مان|(\s|^)تان|(\s|^)شان) ([^ ])/g,
        replacement
      )
  ...
  function fixSuffixSpacing(text) {
    ...
    return (
      text
        ...
        // *am *at *ash *ei *eid *eem *and *man *tan *shan *bi
        .replace(
          newRegExp(
            "([" +
              charsPersian +
              charsDiacritic +
              "]) ((ام|ات|اش|ای|اید|ایم|اند|مان|تان|شان|بی)[" +
              patternAfter +
              "])"
          ),
          replacement
  ...
  function fixDiacritics(text) {
    return (
      text
        ...
        // Props @languagetool-org
        // It's BUGGY: Removes duplicate diacritics but unintentionally deletes the last Tashdid
        // .replace(newRegExp("(.*)([" + charsDiacritic + "]){2,}(.*)"), "$1$2$3")
        .replace(newRegExp("([" + charsDiacritic + "])\\1+"), "$1") // Replace multiple consecutive identical diacritics with a single one
